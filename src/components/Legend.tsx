...
export default function Legend() {
    ...
    return (
        <div className={`legend-root ${isExpanded ? 'expanded' : 'collapsed'}`}>
            <button
                className="legend-toggle-tab"
                onClick={() => setIsExpanded(!isExpanded)}
                title={isExpanded ? "범례 접기" : "범례 펼치기"}
            >
                {isExpanded ? '▼' : '▲'} (범례)
            </button>
            <div className={`legend-content ${isExpanded ? 'show' : ''}`}>
